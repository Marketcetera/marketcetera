...
class RubyStructureDiffViewer extends StructureDiffViewer {
	...
	static class ChangePropertyAction extends Action {
		...
		public ChangePropertyAction(ResourceBundle bundle, CompareConfiguration cc, String rkey, String pkey) {
			...
			fBundle= bundle;
			fPrefix= rkey;
			RubyCompareUtilities.initAction(this, fBundle, fPrefix);
			setCompareConfiguration(cc);
		}

		public void run() {
			boolean b= !RubyCompareUtilities.getBoolean(fCompareConfiguration, fPropertyKey, false);
			setChecked(b);
		...
		public void setChecked(boolean state) {
			...
			RubyCompareUtilities.initToggleAction(this, fBundle, fPrefix, state);
		}
		
		public void setCompareConfiguration(CompareConfiguration cc) {
			fCompareConfiguration= cc;
			setChecked(RubyCompareUtilities.getBoolean(fCompareConfiguration, fPropertyKey, false));
		}
	}
					...
					if (sortedChildren != null && sortedChildren.length > 0) {
						for (int j= 0; j < sortedChildren.length; j++) {
							...
							if (o instanceof DiffNode) {
								...
								if (e instanceof RubyNode) {
									RubyNode jn= (RubyNode) e;
									int tc= jn.getTypeCode();
									if (tc == RubyNode.CLASS || tc == RubyNode.MODULE) {
										firstClass= dn;
									}
								}
							}
	...
	protected void postDiffHook(Differencer differencer, IDiffContainer root) {
		if (fStructureCreator.canRewriteTree()) {
			boolean smart= RubyCompareUtilities.getBoolean(getCompareConfiguration(), SMART, false);
			if (smart && root != null)
