...
public class Selection {
	...
	public boolean covers(Node node) {
		int nodeStart= node.getPosition().getStartOffset();
		return fStart <= nodeStart && node.getPosition().getEndOffset() <= fExclusiveEnd;
	}
	
	public boolean coveredBy(Node node) {
		int nodeStart= node.getPosition().getStartOffset();
		return nodeStart <= fStart && fExclusiveEnd <= node.getPosition().getEndOffset();
	}
	...
	public boolean endsIn(Node node) {
		int nodeStart= node.getPosition().getStartOffset();
		return nodeStart < fExclusiveEnd && fExclusiveEnd < node.getPosition().getEndOffset();
	}
