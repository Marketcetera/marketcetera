...
public class ContentAssistInvocationContext {
	...
	private String fStatementPrefix;
	...
	/**
	 * Computes the identifier (as specified by {@link RubyConventions#isRubyIdentifierPart(char)}) that
	...
	public CharSequence computeIdentifierPrefix() throws BadLocationException {
		if (fPrefix == null) {
			...
			int start= end;
			while (--start >= 0) {
				if (!RubyConventions.isRubyIdentifierPart(document.getChar(start)))
					break;
			}
			start++;
	...
	public CharSequence computeStatementPrefix() throws BadLocationException {
		if (fStatementPrefix == null) {
			IDocument document= getDocument();
			if (document == null)
				return null;
			int end= getInvocationOffset();
			...
			start++;
			fStatementPrefix= document.get(start, end - start);
		}
		
		return fStatementPrefix;
	}
