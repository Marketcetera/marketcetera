...
public class SearchUtil {
	...
	// Settings store
	private static final String DIALOG_SETTINGS_KEY= "RubyElementSearchActions"; //$NON-NLS-1$
	 ...
	 */
	static IRubyScript findCompilationUnit(IRubyElement element) {
		if (element == null)
			return null;
		return (IRubyScript) element.getAncestor(IRubyElement.SCRIPT);
	}
	...
	private static IDialogSettings getDialogStoreSection() {
		IDialogSettings settingsStore= RubyPlugin.getDefault().getDialogSettings().getSection(DIALOG_SETTINGS_KEY);
		if (settingsStore == null)
			settingsStore= RubyPlugin.getDefault().getDialogSettings().addNewSection(DIALOG_SETTINGS_KEY);
