...
public class DepthOfBookManagerTest extends DataFlowManagerTestBase<MDDepthOfBook, DepthOfBookKey> {
	...
	public void testMultipleEvents() throws Exception {
		...
		validateQuotes(mItem1.getBids(), "A", "1", "2", "A", "4", "5", "B", "5", "5");
	...
	public void testChangeAndDelete() throws Exception {
		...
		emit(bid1);
		validateQuotes(mItem1.getBids(), "B", "1", "2");
		validateQuotes(mItem1.getAsks());
		emit(ask1);
		validateQuotes(mItem1.getBids(), "B", "1", "2");
		validateQuotes(mItem1.getAsks(), "A", "3", "4");
		emit(changeBid(bid1, 3));
		validateQuotes(mItem1.getBids(), "B", "1", "3");
		validateQuotes(mItem1.getAsks(), "A", "3", "4");
		emit(changeAsk(ask1, 6));
		validateQuotes(mItem1.getBids(), "B", "1", "3");
		validateQuotes(mItem1.getAsks(), "A", "3", "6");
		emit(AskEvent.deleteEvent(ask1));
		validateQuotes(mItem1.getBids(), "B", "1", "3");
		validateQuotes(mItem1.getAsks());
		emit(BidEvent.deleteEvent(bid1));
		validateQuotes(mItem1.getBids());
		validateQuotes(mItem1.getAsks());
	}
