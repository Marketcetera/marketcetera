		...
		if (memento != null) {
			...
			if (filters.length > 0) {
				...
				for (IMemento filter : filters) {
					mFilters.put(Grouping.valueOf(filter.getString("grouping")), filter.getString("value"));
				}
			}
		}
		updatePartName();
	...
	public void saveState(IMemento memento) {
		...
		if (mFilters != null) {
			for (Map.Entry<Grouping, String> entry : mFilters.entrySet()) {
				...
				filter.putString("grouping", entry.getKey().name());
				filter.putString("value", entry.getValue());
			}
		}
	}
