...
import org.rubypeople.rdt.internal.ui.util.TypeInfoLabelProvider;
import org.rubypeople.rdt.ui.dialogs.ITypeSelectionComponent;
...
public class TypeSelectionComponent extends Composite implements ITypeSelectionComponent {
	...
	private static final String DIALOG_SETTINGS= "org.rubypeople.rdt.internal.ui.dialogs.TypeSelectionComponent"; //$NON-NLS-1$
	...
	private class ToggleStatusLineAction extends Action {
		public ToggleStatusLineAction() {
			super(RubyUIMessages.TypeSelectionComponent_show_status_line_label, IAction.AS_CHECK_BOX);
		}
		public void run() {
			if (fForm == null)
				return;
	...
	private class FullyQualifyDuplicatesAction extends Action {
		public FullyQualifyDuplicatesAction() {
			super(RubyUIMessages.TypeSelectionComponent_fully_qualify_duplicates_label, IAction.AS_CHECK_BOX);
		}
		public void run() {
			boolean checked= isChecked();
	...
	public TypeSelectionComponent(Composite parent, int style, String message, boolean multi, 
			IRubySearchScope scope, int elementKind, String initialFilter, ITitleLabel titleLabel,
	...
	private void createContent(final String message, int elementKind) {
		...
		label.setFont(font);
		label.setText(RubyUIMessages.TypeSelectionComponent_label);
		...
		table.getAccessible().addAccessibleListener(new AccessibleAdapter() {
			public void getName(AccessibleEvent e) {
				if (table.getSelectionCount() == 0) {
					e.result= Strings.removeMnemonicIndicator(RubyUIMessages.TypeSelectionComponent_label);
				}
			}
		});
	...
	private void createViewMenu(Composite parent) {
		...
		fToolItem.setImage(RubyPluginImages.get(RubyPluginImages.IMG_ELCL_VIEW_MENU));
		fToolItem.setDisabledImage(RubyPluginImages.get(RubyPluginImages.IMG_DLCL_VIEW_MENU));
		fToolItem.setToolTipText(RubyUIMessages.TypeSelectionComponent_menu);
...
//						} else {
//							fScope= RubySearchScopeFactory.getInstance().createRubySearchScope(ws, true);
