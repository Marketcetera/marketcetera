...
public final class ProposalSorterHandle {
	...
	private static final String PERFORMANCE_EVENT= RubyPlugin.getPluginId() + "/perf/content_assist_sorters/extensions"; //$NON-NLS-1$
	/**
	...
	private Status createExceptionStatus(InvalidRegistryObjectException x) {
		...
		return new Status(IStatus.INFO, RubyPlugin.getPluginId(), IStatus.OK, disable + " " + reason, x); //$NON-NLS-1$
	}
	...
	private Status createExceptionStatus(CoreException x) {
		...
		return new Status(IStatus.ERROR, RubyPlugin.getPluginId(), IStatus.OK, disable + " " + reason, x); //$NON-NLS-1$
	}
	...
	private Status createExceptionStatus(RuntimeException x) {
		...
		return new Status(IStatus.WARNING, RubyPlugin.getPluginId(), IStatus.OK, disable + " " + reason, x); //$NON-NLS-1$
	}
	...
	private Status createAPIViolationStatus(String operation) {
		...
		return new Status(IStatus.WARNING, RubyPlugin.getPluginId(), IStatus.OK, disable + " " + reason, null); //$NON-NLS-1$
	}
	...
	private String createBlameMessage() {
		...
		String disable= Messages.format(RubyTextMessages.ProposalSorterHandle_blame, args);
		return disable;
	}
