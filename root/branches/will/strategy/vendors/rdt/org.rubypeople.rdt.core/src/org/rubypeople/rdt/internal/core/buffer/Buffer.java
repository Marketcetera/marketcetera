...
public class Buffer implements IBuffer {
	...
	public char getChar(int position) {
		synchronized (this.lock) {
			...
			if (position < this.gapStart) { return this.contents[position]; }
	...
	public char[] getCharacters() {
		synchronized (this.lock) {
			...
			if (this.gapStart < 0) { return this.contents; }
	...
	public String getText(int offset, int length) {
		synchronized (this.lock) {
			...
			if (offset + length < this.gapStart) return new String(this.contents, offset, length);
		...
		if (this.changeListeners != null) {
			for (int i = 0, size = this.changeListeners.size(); i < size; ++i) {
				...
				SafeRunner.run(new ISafeRunnable() {
					...
					// Util.log(exception, "Exception occurred in listener of
					// buffer change notification"); //$NON-NLS-1$
					}
	...
	public void save(IProgressMonitor progress, boolean force) throws RubyModelException {
		...
		if (isReadOnly() || this.file == null) { return; }
	...
	public void setContents(char[] newContents) {
		...
		if (!isReadOnly()) {
			...
			synchronized (this.lock) {
				if (this.contents == null) return; // ignore if buffer is
				// closed (as per spec)
				this.contents = newContents;
				this.flags |= F_HAS_UNSAVED_CHANGES;
				this.gapStart = -1;
	...
	public String toString() {
		...
		buffer.append("Owner: " + ((RubyElement) this.owner).toString()); //$NON-NLS-1$
