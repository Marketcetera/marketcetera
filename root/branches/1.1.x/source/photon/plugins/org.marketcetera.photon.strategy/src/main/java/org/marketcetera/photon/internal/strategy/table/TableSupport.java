	...
	public void createTable(Composite parent) {
		...
		for (int i = 0; i < descriptors.length; i++) {
			...
			if (mConfiguration.isHeaderVisible()) {
				c.setText(descriptors[i].getHeading());
			}
			c.setResizable(descriptors[i].isResizable());
			c.setMoveable(descriptors[i].isMovable());
			c.addSelectionListener(listener);
			layout.setColumnData(c, descriptors[i].getLayoutData());
			beanProperties[i] = descriptors[i].getBeanProperty();
