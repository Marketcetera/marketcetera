        ...
        implements Exchange<SimulatedExchange.Token>
{
     ...
     *  must conform to the requirements established for {@link OrderBook#OrderBook(org.marketcetera.core.trade.Instrument,int)}.
    ...
    public List<DividendEvent> getDividends(ExchangeRequest inExchangeRequest)
    {
        ...
        try {
            ...
            if(inExchangeRequest.getInstrument() == null) {
                throw new IllegalArgumentException(Messages.DIVIDEND_REQUEST_MISSING_INSTRUMENT.getText(inExchangeRequest.toString()));
            }
    ...
    public synchronized void start(List<QuoteEvent> inEvents)
    {
        ...
        if(inEvents != null &&
           !inEvents.isEmpty()) {
            Messages.STARTING_SCRIPTED_EXCHANGE.info(SimulatedExchange.class,
                                            getName());
            setStatus(Status.SCRIPTED);
            doScriptedTicks(inEvents);
            setStatus(Status.COMPLETE);
        } else {
            Messages.STARTING_RANDOM_EXCHANGE.info(SimulatedExchange.class,
                                          getName());
            ...
            ticker = executor.scheduleAtFixedRate(new Runnable()
            {
                public void run()
                {
                    try {
                        ...
                        Messages.SIMULATED_EXCHANGE_TICK_ERROR.warn(SimulatedExchange.class,
                                                           e,
                                                           getName());
                    }
                }
    ...
    public synchronized void stop()
    {
        try {
            ...
            Messages.STOPPING_SIMULATED_EXCHANGE.info(SimulatedExchange.class,
                                             getName());
     ...
     * to complete successfully.  The caller must make sure that {@link #updateInfo(org.marketcetera.core.event.HasInstrument)}
     ...
     *
     * @param inStatus <code>Status</code> value
     */
     ...
     * {@link #updateInfo(org.marketcetera.core.event.HasInstrument)} is called before invoking this method.
     *
        ...
        for(QuoteEvent event : inScriptedEvents) {
            try {
                ...
                if(!getCode().equals(event.getExchange())) {
                    throw new IllegalArgumentException(Messages.SIMULATED_EXCHANGE_CODE_MISMATCH.getText(this,
                                                                                                event,
                                                                                                event.getExchange(),
                                                                                                getCode()));
                }
                ...
                Messages.SIMULATED_EXCHANGE_SKIPPED_EVENT.warn(SimulatedExchange.class,
                                                      e,
                                                      this,
                                                      event);
            }
        }
        Messages.SIMULATED_EXCHANGE_OUT_OF_EVENTS.info(SimulatedExchange.class,
                                              getName());
    }
    /**
        ...
        private class PrivateInstrumentInfo
    {
         ...
         *
         * @param inValue <code>BigDecimal</code> value
         */
         ...
         *
         * @param inLatestTrade <code>TradeEvent</code> value
         */
        ...
        private static class SharedInstrumentInfo
    {
         ...
         *
         * @param inMostRecentValue <code>BigDecimal</code> value
         */
