...
public class OrderReplaceTest extends TypesTestBase {
    ...
    public void pojoDefaults() throws Exception {
        ...
        org.marketcetera.core.trade.TimeInForce fillOrKill = org.marketcetera.core.trade.TimeInForce.FillOrKill;
        ...
        report = createExecReport(orderID, side, orderQty, price,
                ...
                destOrderID, org.marketcetera.core.trade.OrderCapacity.Agency, org.marketcetera.core.trade.PositionEffect.Open);
        //Create the order from the report.
        order = sFactory.createOrderReplace(
                sFactory.createExecutionReport
                (report, cID, Originator.Server, null, null));
        assertOrderReplace(order, NOT_NULL, new org.marketcetera.core.trade.OrderID(orderID),
                destOrderID, OrderType.Limit, side,
                orderQty, price,
                instrument, instrument.getSecurityType(), fillOrKill, account, text,
                cID, org.marketcetera.core.trade.PositionEffect.Open, org.marketcetera.core.trade.OrderCapacity.Agency, null);
        //Verify toString() doesn't fail
        order.toString();
     ...
     * {@link org.marketcetera.core.trade.Factory#createOrderReplace(quickfix.Message, BrokerID)}
     *
     * @throws Exception if there were errors.
     */
    @Test
    public void systemFIXWrap() throws Exception {
        ...
        msg = factory.newCancelReplaceFromMessage(createExecReport(
                orderID.getValue(), org.marketcetera.core.trade.Side.Buy, qty, new BigDecimal("45.67"), price, instrument,
                account, text, OrderType.Limit, org.marketcetera.core.trade.TimeInForce.AtTheClose,
                destOrderID, org.marketcetera.core.trade.OrderCapacity.Individual, positionEffect));
        order = sFactory.createOrderReplace(msg, brokerID);
        assertOrderReplace(order, expectedOrderID, orderID, destOrderID,
                OrderType.Limit, org.marketcetera.core.trade.Side.Buy, qty,
                msg.getField(new Price()).getValue(), instrument, securityType,
                org.marketcetera.core.trade.TimeInForce.AtTheClose, account, text, brokerID, positionEffect,
                org.marketcetera.core.trade.OrderCapacity.Individual, null);
        //Verify toString() doesn't fail
        order.toString();
        checkSetters(order);
        ...
        org.marketcetera.core.trade.OrderCapacity orderCapacity = org.marketcetera.core.trade.OrderCapacity.Proprietary;
        msg = factory.newCancelReplaceFromMessage(createExecReport(
    ...
    public static void setupCurrentFIXDictionary() throws Exception {
        FIXDataDictionaryManager.initialize(FIXVersion.FIX44, FIXVersion.FIX44.getDataDictionaryURL());
    ...
    private Message createExecReport(String inOrderID,
                                     org.marketcetera.core.trade.Side inSide,
                                     BigDecimal inQty,
                                     BigDecimal inPrice,
                                     ...
                                     org.marketcetera.core.trade.TimeInForce inFillOrKill,
                                     String inDestOrderID,
                                     org.marketcetera.core.trade.OrderCapacity inOrderCapacity,
                                     org.marketcetera.core.trade.PositionEffect inPositionEffect)
