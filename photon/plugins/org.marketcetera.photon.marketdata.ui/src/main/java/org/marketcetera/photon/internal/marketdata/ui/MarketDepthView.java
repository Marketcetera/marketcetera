 ...
 */
@ClassVersion("$Id$")
public class MarketDepthView
        extends ViewPart
{
    /**
     * Create a new MarketDepthView instance.
     */
    public MarketDepthView()
    {
        marketDataManager = Activator.getMarketDataManager().getMarketData();
        symbolResolver = Activator.getSymbolResolver();
    }
    /* (non-Javadoc)
            ...
            throws PartInitException
    {
        ...
        symbol = params[0];
        product = Content.valueOf(params[1]);
    }
    @Override
    public void createPartControl(Composite parent)
    {
        ...
        Composite composite = new Composite(sc2,
                                            SWT.NONE);
        ...
        getViewSite().registerContextMenu(BIDS_TABLE_POPUP,
                                          menuManager,
                                          mBidsTable.getTableViewer());
        mBidsTable.getTableViewer().getTable().setMenu(menuManager.createContextMenu(mBidsTable.getTableViewer().getTable()));
        ...
        getViewSite().registerContextMenu(ASKS_TABLE_POPUP,
                                          menuManager2,
                                          mAsksTable.getTableViewer());
        mAsksTable.getTableViewer().getTable().setMenu(menuManager2.createContextMenu(mAsksTable.getTableViewer().getTable()));
        sc2.setMinSize(composite.computeSize(SWT.DEFAULT, SWT.DEFAULT).x, 0);
        hookupData();
    }
    /**
     * Connects market depth data to the view tables.
     */
    private void hookupData()
    {
        mDataItem = marketDataManager.getDepthOfBook(symbolResolver.resolveSymbol(symbol),
                                                     product);
        ObservableListContentProvider bids = new ObservableListContentProvider();
        mBidsTable.getTableViewer().setContentProvider(bids);
        mBidsTable.getTableViewer().setLabelProvider(new ObservableMapLabelProvider(ObjectArrays.concat(createTimeMap(bids),
        ...
        setSortColumn(mBidsTable,
                      new BidComparator(),
                      3,
                      SWT.DOWN);
        mBidsTable.getTableViewer().setInput(EMFObservables.observeList(mDataItem.get(),
        ...
        mAsksTable.getTableViewer().setLabelProvider(new ObservableMapLabelProvider(ObjectArrays.concat(EMFObservables.observeMaps(asks.getKnownElements(),
                                                                                                                                   new EStructuralFeature[] {
                                                         MDPackage.Literals.MD_QUOTE__PRICE,
                                                         MDPackage.Literals.MD_QUOTE__SIZE,
                                                         MDPackage.Literals.MD_QUOTE__SOURCE }), createTimeMap(asks))));
        setSortColumn(mAsksTable,
                      new AskComparator(),
                      0,
                      SWT.UP);
        mAsksTable.getTableViewer().setInput(EMFObservables.observeList(mDataItem.get(),
    ...
    private Composite createHeading(Composite parent) {
        ...
        {
            CLabel text = new CLabel(composite, SWT.BORDER);
            text.setText(getProductLabel(product));
            text.setLayoutData("growx, w 120"); //$NON-NLS-1$
        }
     ...
     */
    private final IMarketData marketDataManager;
    /**
    ...
    /**
     * data item from which market data flows
     */
