...
class ProfitAndLoss
  ...
  @pnl_equity_query =
    ...
    '	WHERE journals.post_date > ? AND journals.post_date <= ? AND postings.sub_account_id=2 AND tradeable_type="Equity" '+
    '	UNION ALL '+
    ' '+
    '	/* phantom cash flows for start date */ '+
    '	SELECT -1*mark_value * PositionTable.position as quantity, marks.currency_id, PositionTable.account_id, PositionTable.strategy '+
    '	FROM marks JOIN '+
    '	( '+
    ...
    '	SELECT mark_value * PositionTable.position as quantity, marks.currency_id, PositionTable.account_id, PositionTable.strategy '+
    '	FROM marks JOIN '+
    '	( '+
    '		SELECT sum(position_qty) AS position, tradeable_id, account_id, strategy, ? as position_date '+
    '		FROM trades JOIN journals ON journals.id=trades.journal_id '+
  ...
  @pnl_forex_query = 
    ...
    ' FROM postings JOIN journals ON postings.journal_id=journals.id '+
    '         JOIN trades ON trades.journal_id=journals.id '+
    ' WHERE journals.post_date > ? AND journals.post_date <= ? AND postings.sub_account_id=2 AND tradeable_type="CurrencyPair" '+
    ' UNION ALL '+
    ' /* phantom cash flows for end date */ '+
  ...
  def initialize(profit_and_loss, currency_id, account_id, strategy)
