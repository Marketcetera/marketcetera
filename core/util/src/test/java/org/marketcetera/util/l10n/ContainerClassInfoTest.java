 ...
 * @since 0.6.0
 * @version $Id: ContainerClassInfoTest.java 16063 2012-01-31 18:21:55Z colin $
 */
    ...
    extends TestCaseBase
{
    ...
    private static class NonstandardFields
    {
        ...
        protected static I18NMessage0P M0_MSG = new I18NMessage0P(LOGGER,
                                                                  "m0");
        I18NMessage1P M1_MSG = new I18NMessage1P(LOGGER,
                                                 "m1");
    ...
    public void nonstandardFields()
        throws Exception
    {
        String category = ContainerClassInfo.class.getName();
        setLevel(category,
                 Level.INFO);
        ActiveLocale.setProcessLocale(Locale.ROOT);
        ContainerClassInfo info = new ContainerClassInfo(NonstandardFields.class);
        assertEquals(NonstandardFields.class,
                     info.getContainer());
        assertEquals(NonstandardFields.PROVIDER,
                     info.getProvider());
        assertArrayPermutation(new I18NMessageInfo[] { new I18NMessageInfo("m0.msg",
                                                                           0,
                                                                           NonstandardFields.M0_MSG) },
                               info.getMessageInfo().toArray(I18NMessageInfo.EMPTY_ARRAY));
        Iterator<LoggingEvent> events = getAppender().getEvents().iterator();
        assertEvent(events.next(),
                    Level.INFO,
                    category,
                    "Non-static field 'M1_MSG' is ignored",
                    category);
        assertEvent(events.next(),
                    Level.INFO,
                    category,
                    "Null field 'M2_MSG' is ignored",
                    category);
