 ...
 *   That way, a piggy resource can't shut down the whole pool.  The corollary of this, of course,
 ...
 */ 
@ClassVersion("$Id$")
public abstract class ResourcePool<ResourceClazz extends Resource>
{    
    /**
     ...
     * 
     * @param inBlock an <code>ExecutableBlock&lt;ResourceClazz,ReturnClazz&gt;</code> value
     * @return a <code>ReturnClazz</code> value
     * @throws ResourcePoolException if an error occurs during the allocation of the resource or the execution of the block
     */
    public <ReturnClazz> ReturnClazz execute(ExecutableBlock<ResourceClazz,ReturnClazz> inBlock) 
    ...
    public void shutdown()
    {
        ...
        synchronized(getPoolLock()) {
            ...
            Iterator<ResourceClazz> poolIterator = getPoolIterator();
     ...
     */
    public abstract void addResourceToPool(ResourceClazz inResource);
    /**
     * Requests a resource from the pool.
     * 
     * Returns the next <code>Resource</code> in the pool as determined by the specific subclass.  
     * If no <code>Resource</code> objects exist in the pool, one will be added.  Accordingly, the execution
     * time of this method may vary.
     * 
     * <p>It is the caller's responsibility to return the <code>Resource</code> to the pool.
     *
     * @param inData an <code>Object/code> value containing any information that might be relevant to the underlying implementation to assist in retrieving the next {@link Resource}
            ...
            throws ResourcePoolException
    {
        ...
        try {
            ...
            synchronized(getPoolLock()) {
                ResourceClazz resource = getNextResource(inData);
                try {
                    resource.allocated();
                } catch (Exception t) {
     ...
     * 
     * @param inResource a <code>Resource</code> value
     * @throws <code>ReturnedPoolException</code> if an error occurs while returning the <code>Resource</code>
     */
            ...
            throws ResourcePoolException
    {
        ...
        synchronized(getPoolLock()) {
            ...
            try {
                ...
                SLF4JLoggerProxy.debug(this, e, "Resource {} LOGIN_FAILED when trying to report its state, releasing", inResource); //$NON-NLS-1$
                shouldRelease = true;
            }
            ...
            try {
                ...
                SLF4JLoggerProxy.debug(this, e, "The resource pool decided not to allow resource {} back in the pool, releasing", inResource); //$NON-NLS-1$
                shouldRelease = true;
            }
     ...
     * as unusable.  This <code>Resource</code> will be taken out of service.
     * 
     ...
     */
    protected void setStatus(Status inStatus)
    {
        synchronized(status) {
            SLF4JLoggerProxy.debug(this, "Resource pool {} status changing to {}", this, inStatus); //$NON-NLS-1$
            status = inStatus;
        }
    }
    /**
     ...
     */
    protected abstract Iterator<ResourceClazz> getPoolIterator();
    /**
     ...
     */
    protected abstract ResourceClazz getNextResource(Object inData) 
            throws ResourcePoolException;
    /**
     ...
     */
    protected abstract ResourceClazz createResource(Object inData)
            throws ResourcePoolException;
    /**
     ...
     * @return a <code>boolean</code> value
     */
    protected abstract boolean poolContains(ResourceClazz inResource);
    /**
     ...
     * 
     * @param inResource a <code>ResourceClazz</code> value being returned to the pool
     * @throws ResourcePoolException if the <code>Resource</code> being returned should be discarded instead.
     */
    protected void verifyResourceReturn(ResourceClazz inResource)
            throws ResourcePoolException
    {
        if(inResource == null) {
    ...
    /**
     * Indicates the status of the pool.
     *
     * @author <a href="mailto:colin@marketcetera.com">Colin DuPlantis</a>
     * @version $Id$
     * @since $Release$
