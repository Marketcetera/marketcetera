...
/**
 * A convenience class that implements common functionality for all {@link ca.odell.glazedlists.EventList}s.
 *
 * <p>If you are creating a custom {@link ca.odell.glazedlists.EventList}, consider extending the more
 * feature-rich {@link ca.odell.glazedlists.TransformedList}.
 *
...
public abstract class AbstractEventList<E> implements EventList<E> {
     ...
     * {@link ca.odell.glazedlists.event.ListEventPublisher}.
     *
     * @param publisher the channel for event distribution. If this is <tt>null</tt>,
     *      then a new {@link ca.odell.glazedlists.event.ListEventPublisher} will be created.
     */
     ...
     *
     * <p>The returned {@link java.util.Iterator} will become inconsistent if the
     * {@link ca.odell.glazedlists.EventList} that it views is modified. To overcome this problem,
     * use {@link #listIterator()}. When used concurrently, the returned
     * {@link java.util.Iterator} requires locking via this list's
     ...
     * sequence).
     *
     * <p>Unlike the {@link java.util.ListIterator} from a regular {@link java.util.List}, the
     * {@link ca.odell.glazedlists.EventList}'s {@link java.util.ListIterator} will remain consistent even if the
     * {@link ca.odell.glazedlists.EventList} is changed externally. Note that when used concurrently,
     * the returned {@link java.util.ListIterator} requires locking via this list's
     * {@link #getReadWriteLock() ReadWriteLock}.
     *
     ...
     * <p>Unlike the standard {@link java.util.List#subList(int,int) List.subList()}
     * method, the {@link java.util.List} returned by this method will continue to be
     * consistent even if the {@link ca.odell.glazedlists.EventList} it views is modified,
     * structurally or otherwise. The returned {@link java.util.List} can always be safely
     * cast to {@link ca.odell.glazedlists.EventList}. Note that when used concurrently, the returned
     * {@link java.util.List} requires locking via this list's
     * {@link #getReadWriteLock() ReadWriteLock}.
     *
