...
import org.rubypeople.rdt.internal.ui.preferences.TodoTaskConfigurationBlock.TodoTask;
...
public class TodoTaskInputDialog extends StatusDialog {
	...
	public TodoTaskInputDialog(Shell parent, TodoTask task, List existingEntries) {
		...
		if (task != null) {
			if (RubyCore.COMPILER_TASK_PRIORITY_HIGH.equals(task.priority)) {
				fPriorityDialogField.selectItem(0);
			} else if (RubyCore.COMPILER_TASK_PRIORITY_NORMAL.equals(task.priority)) {
	...
	public TodoTask getResult() {
		...
		switch (fPriorityDialogField.getSelectionIndex()) {
			case 0 :
					task.priority= RubyCore.COMPILER_TASK_PRIORITY_HIGH;
				break;
			case 1 :
					task.priority= RubyCore.COMPILER_TASK_PRIORITY_NORMAL;
				break;
			default :
					task.priority= RubyCore.COMPILER_TASK_PRIORITY_LOW;
				break;				
		}
		return task;
	}
