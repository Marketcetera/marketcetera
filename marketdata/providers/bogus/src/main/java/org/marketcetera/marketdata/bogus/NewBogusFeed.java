        ...
        extends AbstractMarketDataProvider
{
    ...
    private static class Request
    {
         ...
         * @param inParentFeed a <code>NewBogusFeed</code> value
         * @return a <code>String</code> value
         */
        private static String execute(MarketDataRequest inRequest,
                                      MarketDataRequestAtom inRequestAtom,
                                      NewBogusFeed inParentFeed)
        {
         ...
         * @param inRequest a <code>MarketDataRequest</code> value
         * @param inRequestAtom a <code>MarketDataRequestAtom</code> value
         * @param inFeed a <code>NewBogusFeed</code> value
         */
        ...
        private synchronized void execute()
        {
            ...
            try {
                ...
                if(marketDataRequestAtom.getInstrument() != null) {
                    if(marketDataRequestAtom.getInstrument().getSecurityType() == SecurityType.CommonStock) {
                        //                    exchangeRequests.add(ExchangeRequestBuilder.newRequest().withInstrument(marketDataRequestAtom.getInstrument()).create()); TODO - instrument incompatibility
                    } else if(marketDataRequestAtom.getInstrument().getSecurityType() == SecurityType.Future) {
                        //                        exchangeRequests.add(ExchangeRequestBuilder.newRequest().withInstrument(Future.fromString(symbol)).create()); TODO - instrument incompatibility
                    } else if(marketDataRequestAtom.getInstrument().getSecurityType() == SecurityType.Future) {
                        // TODO instrument incompatability
                        ...
                        // this is a new security type and there is no support for it here
                        throw new UnsupportedOperationException();
                    }
                } else {
                    // marketDataRequestAtom has no instrument - should then have underlyingInstrument instead
                    //                exchangeRequests.add(ExchangeRequestBuilder.newRequest().withUnderlyingInstrument(marketDataRequestAtom.getUnderlyingInstrument()).create()); TODO instrument incompatability
                }
         ...
         */
        private final NewBogusFeed feed;
        /**
