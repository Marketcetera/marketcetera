 ...
 * undeploys the currently selected strategy.
 * 
 ...
 */
@ClassVersion("$Id$")
public final class UndeployHandler extends SafeHandler {
        ...
        final IRunnableWithProgress operation = JFaceUtils.wrap(
                new Callable<Void>() {
                    ...
                    public Void call() throws Exception {
                        connection.undeploy(strategy);
                        return null;
                    }
                }, Messages.UNDEPLOY_HANDLER__TASK_NAME.getText(name));
        ProgressUtils
                .runModalWithErrorDialog(HandlerUtil
                        .getActiveWorkbenchWindowChecked(event), operation,
                        new I18NBoundMessage1P(
                                Messages.UNDEPLOY_HANDLER_FAILED, name));
    }
}
